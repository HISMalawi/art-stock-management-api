class Pharmacy < ActiveRecord::Base
  ...
  def self.total_removed(drug_id , start_date = Date.today , end_date = Date.today)
    ...
    self.active.where(["pharmacy_encounter_type = ? AND drug_id = ?
      AND encounter_date >= ? AND encounter_date <= ?",
        pharmacy_encounter_type.id , drug_id , start_date , end_date]).select("SUM(value_numeric) total_removed"
  ...
  def Pharmacy.dispensed_drugs_since(drug_id, start_date = Date.today , end_date = Date.today)
    ...
    Encounter.joins("INNER JOIN obs USING(encounter_id)").select("SUM(value_numeric) total_dispensed"
    ).where(["concept_id = ? AND encounter_type = ?
                   AND obs_datetime >= ? AND obs_datetime <= ? AND value_drug = ? AND obs.voided=0",
        amount_dispensed_concept_id,dispensed_encounter.id,
        start_date,end_date,drug_id]).group("value_drug").first.total_dispensed.to_f rescue 0
  end
  ...
  def Pharmacy.dispensed_drugs_to_date(drug_id)
    ...
    Encounter.joins("INNER JOIN obs USING(encounter_id)").select("SUM(value_numeric) total_dispensed"
    ).where(["concept_id = ? AND encounter_type = ? AND value_drug = ? AND obs.voided=0",
        amount_dispensed_concept_id,dispensed_encounter.id,drug_id]).group("value_drug").first.total_dispensed.to_f rescue 0
  end
  ...
  def self.delivered(drug_id, start_date = Date.today, end_date = Date.today)
    ...
    return Pharmacy.where(["pharmacy_encounter_type = ? AND (DATE(encounter_date) BETWEEN (?) AND (?)) AND drug_id = ?",
  ...
  def self.total_delivered(drug_id, start_date = Date.today ,end_date = Date.today)
    ...
    self.active.where(["pharmacy_encounter_type = ? AND drug_id = ? AND encounter_date >= ? AND encounter_date <= ?",
        pharmacy_encounter_type.id , drug_id , start_date , end_date]).select("SUM(value_numeric) total_delivered"
    ).group("drug_id").first.total_delivered.to_f rescue 0
  ...
  def self.expiring_drugs(start_date , end_date)
    ...
    expiring_drugs = self.active.where(["pharmacy_encounter_type = ? AND expiry_date >= ? AND expiry_date <= ?",
  ...
  def self.currently_expiring_drugs(start_date, drug_id)
    ...
    expiring_drugs = self.active.where(["pharmacy_encounter_type = ? AND expiry_date >= ? AND expiry_date <= ? AND drug_id = ?",
  ...
  def self.removed_from_shelves(start_date , end_date)
    ...
    removed_from_shelves = self.active.where(["pharmacy_encounter_type = ? AND encounter_date >= ? AND encounter_date <= ?",
        pharmacy_encounter_type.id , start_date , end_date])
  ...
  def Pharmacy.prescribed_drugs_since(drug_id,start_date,end_date = Date.today)
    ...
    drug_orders = DrugOrder.joins("INNER JOIN orders ON drug_order.order_id = orders.order_id
        INNER JOIN encounter e ON e.encounter_id = orders.encounter_id"
    ).where(["encounter_type = ? AND drug_inventory_id = ?
    ...
    (drug_orders).each do | drug_order |
      order = drug_order.order rescue nil
      next if order.blank?
...
EOF
  ...
  def self.verify_stock_count(drug_id,start_date,end_date, type=nil)
    ...
    encounter_type_id = PharmacyEncounterType.find_by_name('Tins currently in stock').id
    start_date = Pharmacy.active.where(["pharmacy_encounter_type = ? AND encounter_date <= ? AND drug_id = ? #{condition}",
        encounter_type_id, start_date,drug_id]).order("encounter_date DESC,date_created DESC").first.value_numeric rescue 0
  end
  ...
  def self.total_physically_counted(drug_id, start_date ,end_date = Date.today)
    ...
    ).select("SUM(value_numeric) total_physical_count").group("drug_id").first.total_physical_count.to_f rescue 0
  ...
  def self.update_average_drug_consumption(drug_id)
    ...
    pharmacy_obs = Pharmacy.where(["pharmacy_encounter_type =? AND drug_id =? AND value_text = ?", edited_stock_encounter_id, drug_id, 'Drug Rate']).last
